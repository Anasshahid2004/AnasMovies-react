...
const App = () => {
  ...
  const searchMoviesCallback = useCallback(
    async (title, page = 1) => {
      setLoading(true);
    ...
    [movieType, releaseYear]
  );

  useEffect(() => {
    searchMoviesCallback("", 1);
  ...
  const searchMovies = useCallback(
    async (title, page = 1) => {
      searchMoviesCallback(title, page);
    },
    [searchMoviesCallback]
  );
  ...
  return (
    <div className="app">
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <h1 onClick={handleTitleClick}>
          ...
          <span style={{ verticalAlign: "middle" }}>
            ...
            <img
              className="logo"
              src="https://i.imgur.com/l4m41WG.png"
              alt="Number 3"
            />{" "}
          </span>{" "}
          M O V I E S{" "}
      ...
      {movies?.length > 0 ? (
        ...
        <div className="empty">
          <h2>No movies found, Search another title Movies! </h2>
        </div>
      )}
